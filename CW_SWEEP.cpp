...
int sweep(doub g2, doub LineW,doub period,int period_steps,int sweep_steps,doub Max_detune,doub peak_1,doub peak_2,doub convergence,doub convergence_threshold,int conS,int expN,int Msteps, int polarization, doub dLine, int numThread )
{
    ...
    strstream<<"./Data/CW_D"<<dLine<<"_pol_"<<polarization<<"_g2_"<<g2/2/pi<<"_LW_"<<LineW/2/pi<<"_dt_"<<period/period_steps<<"_"<<peak_1<<"_uWcm2_"<<convergence<<"_conS_"<<conS<<"_O="<<nexp<<".txt";
    strstream>>filename;
    cout<<filename.c_str()<<endl;
    file2.open(filename.c_str(),ios::out | ios::trunc);
    strstream2<<"./Data/CW_FS_D"<<dLine<<"_pol_"<<polarization<<"_g2_"<<g2/2/pi<<"_LW_"<<LineW/2/pi<<"dt_"<<period/period_steps<<"_"<<peak_1<<"_uWcm2_"<<convergence<<"_conS_"<<conS<<"_O="<<nexp<<".txt";
    ...
    doub upperLevelJvalue;

    if ( dLine == 1 )
    {
        upperLevelJvalue=0.5;
    }
    else
    {
        upperLevelJvalue=1.5;
    }

    doub polarization1,polarization2;
...
#pragma omp num_threads(numThread)
#pragma omp parallel for
    for (int thread=0;thread<numThread;thread++)
    {
        ...
        for (int m= int(omp_get_thread_num()/2)*sweep_steps;m<=sweep_steps*(int(omp_get_thread_num()/2)+1);m++)
        {
            ...
            cout<<m<<endl;
            doub De=1.0*m*(pow(-1,omp_get_thread_num()))*1.0/sweep_steps/numThread;
                ...
                for (int t=-j;t<j+1;t++)
                    for (int m=3; m<5;m++)
                        for (int n=-m;n<m+1;n++)
                        {
                            if (m==4)
                            {
                                M(D1_coef(1,j,t),D1_coef(0,m,n))=(atom.coef(polarization1,1,0,j,m,t,n,1.5,0.5,3.5)+atom.coef(polarization2,1,0,j,m,t,n,upperLevelJvalue,0.5,3.5))/2*sqrt(peak_1/100)*peakO;
                            }
                            else
                            {
                                M(D1_coef(1,j,t),D1_coef(0,m,n))=(atom.coef(polarization1,1,0,j,m,t,n,1.5,0.5,3.5)+atom.coef(polarization2,1,0,j,m,t,n,upperLevelJvalue,0.5,3.5))/2*sqrt(peak_2/100)*peakO;
                            }

                    ...
                    for (int j=3; j<5;j++)
                        for (int t=-j;t<j+1;t++)
                            for (int m=3; m<5;m++)
                                for (int n=-m;n<m+1;n++)
                                {
                                    M(D1_coef(1,j,t),k*neq+D1_coef(0,m,n))=(atom.coef(polarization1,1,0,j,m,t,n,upperLevelJvalue,0.5,3.5)+atom.coef(polarization2,1,0,j,m,t,n,upperLevelJvalue,0.5,3.5))/2*ReRabi(Time[k],period,peak,A_Factor);
